...
namespace LenseAF.Items
{
    ...
    public class LenseAFInstruction : SequenceItem, IValidatable {
        ...
        public LenseAFInstruction(ICameraMediator camera, IImagingMediator imagingMediator)
        {
            cam = camera;
            med = imagingMediator;
            utility = new Utility();
        ...
        public LenseAFInstruction(LenseAFInstruction copyMe) : this(copyMe.cam, copyMe.med) {
            CopyMetaData(copyMe);
        }
        ...
        public override async Task Execute(IProgress<ApplicationStatus> progress, CancellationToken token)
        {
            ...
            Directory.CreateDirectory(Path.Combine(CoreUtil.APPLICATIONTEMPPATH, "AutoFocus", "Camera Lense AF"));

            File.WriteAllText(Path.Combine(CoreUtil.APPLICATIONTEMPPATH, "AutoFocus", "Camera Lense AF", Guid.NewGuid().ToString() + ".json"), JsonConvert.SerializeObject(FocusPoints, Formatting.Indented));

            return;
        }

        public override object Clone() {
            return new LenseAFInstruction(this);
        }

        public override string ToString() {
            return $"Category: {Category}, Item: {nameof(LenseAFInstruction)}";
        }
