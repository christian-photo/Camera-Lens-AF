...
{
    public class ContrastDetection
    {
        private static int _maxWidth = 1552;

        public ContrastDetectionResult Measure(BitmapSource image, IRenderedImage render, ContrastDetectionParams p, IProgress<ApplicationStatus> progress, CancellationToken token)
        {
            var result = new ContrastDetectionResult();
            try
            {
                var state = GetInitialState(render, image, p);
        ...
        private static State GetInitialState(IRenderedImage renderedImage, BitmapSource image, ContrastDetectionParams p)
        {
            var state = new State();
            ...
            state._originalBitmapSource = image;
